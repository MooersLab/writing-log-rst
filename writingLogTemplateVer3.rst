...
.. _`sub:entry-protocol`:

Daily protocol
--------------

#. At start of work session, review the timeline
   `2.5 <#sub:benchmarks>`__, recent daily entries
   `2.2 <#sub:daily-log>`__, next action item `2.6 <#sub:next>`__, and
   to-do list `2.7 <#sub:to-do>`__.
...
#. Move the unfinished items to the to-do list `2.7 <#sub:to-do>`__.

#. Identify the next task or action `2.6 <#sub:next>`__.
...
#. Update your personal knowledge base `2.4 <#sub:zk>`__.
...
project basis  [1]_. The Notebook automatically updates sums of words
...
section 4 for guidance (`4.7 <#subsec:guides:knowledgebase>`__).
...
.. _`ssubsec:new-ideas:Discussion`:
...
.. _`subsec:guides:overleaf`:

Tips for using Overleaf
-----------------------

#. Chrome has the TextArea extension that is needed to run Grammarly in
   Overleaf.

#. Use the shortcuts (new commands defined in the preamble) to save time
   typing.

#. Where shortcuts are not possible, use templates.

#. View Overleaf project with Chrome to be able to run Grammarly via the
   Chrome Grammarly extension.

#. code Snippets can be mapped to voice commands in Voice In Plus.

.. _`subsec:guides:grammarlyInoverleaf`:
...
#. Replace unicode characters with LaTeX code: e.g., replace Å with Å.
   Not all LaTeX document classes are compatible with unicode.

#. The primes have to be replaced with ’.

#. The vertical red rectangles with a white dot in the middle should be
   replaced with a whitespace.

#. There are two styles in the bibtex world: bibtex and biblatex. We are
   using bibtex. It is simpler. It has fewer fields.

#. Use Google Scholar bibtex over Medline or PubMed biblatex.

#. Often the error is in the bibitem entry above the one indicated in
   the error messages.

#. All interior braces must by followed by a comma, including the last
   one.

#. When stumped, replace the entry with a fresh one from Google Scholar.

.. _`subsec:guides:graphicalAbstract`:

Graphical Abstract
------------------

The following is copied from the Crystal Journal’s `author
guidelines <https://www.mdpi.com/journal/crystals/instructions#preparation>`__.
   ...
   Calibri. The minimum size required for the GA is 560 :math:`\times`
   1100 pixels (height :math:`\times` width). When submitting larger
   images, please, keep to the same ratio.
...
`here <https://www.mdpi.com/2073-4352/11/3/273>`__.

.. _`subsec:guides:benchmarks`:

Guidelines for benchmarks
-------------------------

.. _`subsec:guides:wpnb`:

Guidelines for using Writing Progress Notebook
----------------------------------------------

The writing progress notebook enables the tracking of progress on a
project basis  [3]_. The Notebook automatically updates sums of words
written and minutes spent across all projects on a given day. It only
takes a few seconds to enter the number of words written and the time
spent for a specific project on that project’s Google Sheet. If you have
Voice In plus activated, say the words “open sheet 37” to have the
...
permanent notes at the end of a work session  [5]_  [6]_. The name of
the index for this project is ``XXXXXXXXX``. Enter ``Control-c n f`` to
...
.. [1]
   https://github.com/MooersLab/writing-progress-2024-25

.. [2]
   `<insert link for specific sheet> <<insert link for specific sheet>>`__
...
.. [5]
   https://wiki2.org/en/Zettelkasten

.. [6]
   https://wiki2.org/en/Comparison_of_note-taking_software
